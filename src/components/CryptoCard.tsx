      ...
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center justify-between">
          ...
          {previousPrice && !isLoading && (
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl shadow-md transition-all duration-200 ${
              isPositive
                ? 'bg-green-50 text-green-700 border border-green-200 hover:bg-green-100'
                : 'bg-red-50 text-red-700 border border-red-200 hover:bg-red-100'
            }`}>
              {isPositive ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
              <span className="text-sm font-bold persian-number">
      ...
      <CardContent>
        <div className="space-y-4 text-right">
          <div className="text-center">
            <div className="text-4xl font-bold persian-number mb-2 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
              {isLoading ? (
            ...
            {previousPrice && !isLoading && (
              <div className={`text-lg font-medium persian-number ${
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
            <div className="text-center">
              ...
              <div className={`text-sm font-bold persian-number ${
                isPositive ? 'text-green-600' : 'text-red-600'
              }`}>
                {isPositive ? '+' : ''}{priceChangePercent.toFixed(2)}%
              </div>
